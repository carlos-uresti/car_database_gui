...
#adds tabs to database notebook
...
car_database.add(tab5, text = "Reserve Rental")
...
def submit():
	...
	submit_cur.execute("INSERT INTO RENTAL VALUES (:StartDate, :EndDate, :Year, :Type, :Category) ",
		{
			'VehicleID': vehicle_id.get(),
			'Description': description.get(),
			'Year': year.get(),
...
########################list of vehicles available in specified window
def input_query():
  iq_conn = sqlite3.connect('rental.db')
  iq_cur = iq_conn.cursor()
  ...
  drop3.grid(row = 10, column =0, columnspan = 2, pady = 10, padx = 10, ipadx = 140)
...
#############################vehicle category dropdown menu
rentalTypes = ["1", "7"]
rentalType = StringVar()
rentalType.set("Rental Type")
drop4 = OptionMenu(tab5, rentalType, *rentalTypes)
drop4.grid(column = 1, row= 6)
...
endDate_label = Label(tab5, text = 'Return Date*: ')
...
rentalType_label = Label(tab5, text = 'Rental Type: ')
rentalType_label.grid(row =6, column = 0)

qty_label = Label(tab5, text = 'Quantity: ')
